...
final class BytecodeHelper {
    ...
    public static Opcode getCodeOfTypeAndPrimaryTypeKind(Opcode.Kind opcodeKind, TypeKind typeKind) {
        return Arrays.stream(Opcode.values())
                .filter(code -> code.kind().equals(opcodeKind) && primaryTypeKind(code).equals(typeKind))
                .findFirst().orElseThrow();
    }
    public static Opcode getCodeOfTypeAndTypeKinds(Opcode.Kind opcodeKind, TypeKind primaryTypeKind, TypeKind secondaryTypeKind) {
        return Arrays.stream(Opcode.values())
                .filter(code -> code.kind().equals(opcodeKind) && primaryTypeKind(code).equals(primaryTypeKind)
                    && secondaryTypeKind(code).equals(secondaryTypeKind))
                .findFirst().orElseThrow();
    }

    public static Opcode addOpcode(TypeKind leftTypeKind) {
        leftTypeKind = leftTypeKind.asLoadable();
        return switch (leftTypeKind) {
            case INT -> Opcode.IADD;
            case LONG -> Opcode.LADD;
            case FLOAT -> Opcode.FADD;
            case DOUBLE -> Opcode.DADD;
            default -> throw new IllegalStateException("Unexpected value: " + leftTypeKind);
        };
    }
    
    public static Opcode subOpcode(TypeKind leftTypeKind) {
        leftTypeKind = leftTypeKind.asLoadable();
        return switch (leftTypeKind) {
            case INT -> Opcode.ISUB;
    ...
    public static Opcode mulOpcode(TypeKind leftTypeKind) {
        leftTypeKind = leftTypeKind.asLoadable();
        return switch (leftTypeKind) {
            case INT -> Opcode.IMUL;
    ...
    public static Opcode divideOpcode(TypeKind leftTypeKind) {
        leftTypeKind = leftTypeKind.asLoadable();
        return switch (leftTypeKind) {
            case INT -> Opcode.IDIV;
    ...
    public static Opcode shiftLeftOpcode(TypeKind leftTypeKind) {
        leftTypeKind = leftTypeKind.asLoadable();
        return switch (leftTypeKind) {
            case INT -> Opcode.ISHL;
            case LONG -> Opcode.LSHL;
            default -> throw new IllegalStateException("Unexpected value: " + leftTypeKind);
        };
    }

    public static Opcode shiftRightOpcode(TypeKind leftTypeKind) {
        leftTypeKind = leftTypeKind.asLoadable();
        return switch (leftTypeKind) {
            case INT -> Opcode.ISHR;
            case LONG -> Opcode.LSHR;
            default -> throw new IllegalStateException("Unexpected value: " + leftTypeKind);
        };
    }

    public static Optional<Opcode> compareOpcode(TypeKind leftTypeKind) {
        leftTypeKind = leftTypeKind.asLoadable();
        return Optional.ofNullable(switch (leftTypeKind) {
            case LONG -> Opcode.LCMP;
            case FLOAT -> Opcode.FCMPG;
            case DOUBLE -> Opcode.DCMPG;
            default -> null;
        });
    }
