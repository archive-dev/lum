...
class ClassModelProcessor {
    ...
    public void processClassModel() {
        switch (model.ctx.orElseThrow()) {
            ...
            default -> throw new IllegalStateException("Unexpected value: " + model.ctx);
        }
    }

    private void processClassModelMembers(List<LumParser.DeclarationContext> ctxs) {
        new MemberProcessor(typeProcessor).processMembers(ctxs).toArray(model.members());
    }
    ...
    private void processGenericTypeDeclaration(LumParser.GenericTypeDeclarationContext ctx) {
        ...
        model.setSuperClass(pair.a().isPresent() ? pair.a() : ClassModelFactory.of(Object.class));
