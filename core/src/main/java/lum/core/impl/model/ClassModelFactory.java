...
public final class ClassModelFactory {
    ...
    public static Optional<ClassModel> ofFile(Path workDir, Path path) {
        path = Utils.subtractPaths(path, workDir);
    ...
    private static void processMembers(ClassModel model, Class<?> clazz) {
        ...
        for (int i = 0; i < members.length; i++) {
            ...
            switch (member) {
                case Method method -> model.members()[i] = MethodModelFactory.of(method);
                case Constructor<?> method -> model.members()[i] = MethodModelFactory.of(method);
                case Field field -> model.members()[i] = FieldModelFactory.of(field);
                case null, default -> {}
            }
        }
    }
